...
class ShowCQTElement extends HTMLElement {
    ...
    constructor(custom_ctx) {
	    ...
	    /* --- ĐOẠN CSS BẠN MUỐN THÊM CỨNG VÀO ĐÂY --- */
	    #axis {
                z-index: 10;
            }
		...
		z-index: 9;
            ...
            /* CSS CẮT SÓNG ÂM (Luôn luôn chạy theo biến của thanh trượt) */
            /* Nếu chưa kéo thanh trượt, biến var(--custom-cut) sẽ tự nhận là 0.3vw (không cắt) */
            #canvas {
                width: auto; height: auto;
                clip-path: inset(0 0 var(--custom-cut, 0.3vw) 0);
                transform: translateY(var(--custom-cut, 0.3vw));
            }

            /* CSS ẨN TRỤC (Chỉ chạy khi tích vào nút Hide Axis) */
            :host([data-hide-axis="1"]) #axis,
            :host([data-hide-axis="1"]) #container::after {
    ...
    #create_alpha_table = () => {
        ...
        if (p.height > 0) {
            ...
            for (let y = 0; y < p.height; y++)
                p.alpha_table[y] = (p.opacity == "opaque" || y >= p.bar_h) ?
                    255 :
	...
	let isHideBars = this.dataset.hideBars === "1"; // ĐỌC TRẠNG THÁI NÚT BẤM

        for (let y = 0; y < p.bar_h + p.axis_h; y++) {
            if (y <= p.bar_h)
                p.cqt.render_line_alpha(y, p.alpha_table[y]);

	    // --- BỘ LỌC CHUẨN: XÓA NỀN ĐEN + GIỮ ĐỘ MỊN SÓNG ---
            let out = p.cqt.output;
            for (let i = 0; i < out.length; i += 4) {
		if (isHideBars) {
                    out[i+3] = 0; // NẾU BẬT NÚT -> Ép sóng tàng hình 100%
                } else {
                    // NẾU TẮT NÚT -> Chạy bộ lọc giữ độ mịn sóng như cũ
                    // 1. Tìm giá trị màu sáng nhất trong 3 kênh Đỏ, Xanh Lá, Xanh Dương
                    let brightness = Math.max(out[i], out[i+1], out[i+2]);
                
                    // 2. Lấy giá trị nhỏ nhất giữa độ mờ dọc (hàm sin) và độ sáng của màu.
                    // - Nếu là nền đen (brightness = 0) => Alpha = 0 (tàng hình tuyệt đối).
                    // - Nếu là sóng nhạc => Sóng giữ được độ mượt, không bị viền đen xơ xác.
                    out[i+3] = Math.min(out[i+3], brightness); 
                } 
            }
            // --- KẾT THÚC CHÈN ---
