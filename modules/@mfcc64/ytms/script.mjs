...
(async function(){
	...
	// 1. NÚT CHECKBOX "HIDE AXIS"
        create_child_checkbox_menu("Hide Axis", "hide_axis", (child) => {
            // Đẩy thuộc tính data-hide-axis="1" vào thẻ gốc để CSS xóa trục
            cqt.dataset.hideAxis = child.checked ? "1" : "0";
            
            // KIỂM TRA BẢO VỆ: Chỉ chạy khi thanh trượt Cut Canvas ĐÃ TỒN TẠI
            if (child_menu.custom_cut) {
                if (child.checked) {
                    // Khi tích vào nút -> Ép thanh trượt lên 17 (1.7vw)
                    child_menu.custom_cut.value = 17;
                } else {
                    // Khi bỏ tích nút -> Ép thanh trượt về 3 (0.3vw)
                    child_menu.custom_cut.value = 3;
                }
                // Bắt buộc gọi lệnh này để thanh trượt chạy logic cắt CSS
                child_menu.custom_cut.onchange(); 
            }
        });

        // 2. THANH TRƯỢT "CUT CANVAS" (Độc lập, kéo là cắt)
        create_child_range_menu("Cut Canvas", "custom_cut", (child) => {
            // Chia 10 để ra số thập phân (VD: kéo đến 17 -> 1.7)
            let realValue = (child.value / 10).toFixed(1);
            
            // Bắn trực tiếp biến số này vào style của thẻ gốc. 
            // Ngay khi bạn kéo, CSS sẽ nhận được số mới và cắt lập tức!
            cqt.style.setProperty('--custom-cut', realValue + 'vw');
            
            // Cập nhật con số hiển thị chữ "vw" trên menu cho pro
            let textElement = child.parentElement.nextElementSibling;
            if (textElement) {
                textElement.textContent = realValue + "vw";
            }
        });
	
	// 3. NÚT CHECKBOX "HIDE BARS" (Tàng hình sóng, chỉ để lại Line)
        create_child_checkbox_menu("Hide Bars", "hide_bars", (child) => {
            // Khi tích vào nút, đẩy trạng thái "1" (Bật) sang cho bộ lọc Pixel
            cqt.dataset.hideBars = child.checked ? "1" : "0";
        });
